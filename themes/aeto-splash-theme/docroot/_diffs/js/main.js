...
AUI().ready(
	function(A) {
		var node = A.one("body");
		var nodes = [];
		var width = node.get("winWidth");
		var height = node.get("winHeight");
		var diagonal = Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2));
		var containerNode = A.one('.transition-container');
		var angle = -90 + Math.atan(width / height) * (180 / Math.PI);
		var nodeCt = 0,
		...
		node = A.one(".transition-container");
		node.setStyle('-webkit-transform', "rotate(" + angle + "deg)");
		node.setStyle('-moz-transform', "rotate(" + angle + "deg)");
		node.setStyle('-ms-transform', "rotate(" + angle + "deg)");	
		node.setStyle('-o-transform', "rotate(" + angle + "deg)");
		node.setStyle('transform', "rotate(" + angle + "deg)");
		node.setStyle('height', diagonal);
		node.setStyle('margin-bottom', width * -0.5 + "px");
		node.setStyle('margin-left', height * 0.5 + "px");		
		...
		while (spaceCt < 100) {
			...
			nodes[nodeCt].setStyle('margin-left', diagonal * -1);
			...
			if (bgCss == "") {
				bgCss = nodes[nodeCt].getStyle('background-image');
			...
			if(bgCss != "-webkit-gradient") {
				nodes[nodeCt].setStyle("background-image", bgCss + "(left, hsla(" + randColor + ", 67%, 36%, 0) 0%, hsla(" + randColor + ", 35%, 75%, 1) 32%, #ffffff 54%)");
			}
			else {
				nodes[nodeCt].setStyle("background-image", bgCss + "(linear, 0% 50%, 100% 50%, color-stop(0%, hsla(" + randColor + ", 67%, 36%, 0)), color-stop(32%, hsla(" + randColor + ", 35%, 75%, 1)), color-stop(54%, #ffffff)");
			}
		...
		function wipeStripe() {
			if (nodes.length > 0) {
				...
				nodes[idx].setStyle('margin-left', diagonal * 2)
				nodes.splice(idx, 1);
			} 
			else {
		...
		var menuItems = [A.one('.item .two'), A.one('.item .three'), A.one('.item .four')];
			...
			setTimeout(function() {
				...
				setTimeout(function() {
					...
					setTimeout(function() {
						...
						topLineNode.setStyle('transition-duration', "0.45s");
						...
						bottomLineNode.setStyle('transition-duration', "0.45s");

						menuItems[0].setStyle('opacity', 1);
						menuItems[0].setStyle('top', 0);
...
AUI().use('event-hover', 'transition', function (A) {
    ...
    function over() {
        ...
        A.one('.left-grad').setStyle('left', 0);
        A.one('.right-grad').setStyle('right', 0);
    ...
    function out() {
        ...
        A.one('.left-grad').setStyle('left', 150);
        A.one('.right-grad').setStyle('right', 150);
        A.one('.left-grad').setStyle('opacity', 1);
        A.one('.right-grad').setStyle('opacity', 1);
...
AUI().ready(
	function(A) {
		...
		function myTimer() {
			...
			A.one('.aeto-crystal ').setStyle('background-color', colorCycle[idx]);
			idx++;
		}
